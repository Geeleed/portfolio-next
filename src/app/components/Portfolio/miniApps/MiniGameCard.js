...
function MiniGameCard() {
  ...
  return (
    <div className={css.main}>
      <Begin setOn={setOn} />
      {on ? (
        <Player setOn={setOn} setEnd={setEnd} setGameState={setGameState} />
      ) : (
        <></>
      )}
      {end ? (
        <End setOn={setOn} setEnd={setEnd} gameState={gameState} />
      ) : (
        <></>
      )}
    </div>
  ...
  return (
    <div className={css.begin}>
      ...
      <div
        onClick={() => {
          setHelp(
            <p className={css.howTo} onClick={() => setHelp()}>
              วิธีเล่น: มาเช้คสายตากันหน่อย กด Start แล้วจับคู่สีให้ตรงกัน
              ระวังนะ เฉดสีอาจจะใกล้เคียงกันจนคุณจำผิด!
            </p>
          );
        }}
      >
      ...
      setTimeout(() => {
        if (hexCode[id1] == hexCode[id2]) {
          ...
          document.getElementById(
            id1
          ).style.cssText = `background-color:var(--tiny-color);`;
          document.getElementById(
            id2
          ).style.cssText = `background-color:var(--tiny-color);`;
        }
  ...
  return (
    <div className={css.player}>
      <header>
        ...
        >
          <svg
            style={{ animationDuration: `${count / 15}s` }}
      ...
      <section className={css.section}>
        {hexCode.map((color, index) => (
          <Box color={color} key={index} index={index} process={process} />
        ))}
      </section>
    </div>
  );
