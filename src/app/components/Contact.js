...
function Contact() {
  ...
  return (
    <div id='contact' className={css.main}>
      ...
      <form className={css.form} >
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', }}>
          ...
          <Link href={'https://www.linkedin.com/in/surasak-kaewpho-18b410293/'} target='_blank'>
...
function Poster({ id, data }) {
  return (
    <div className={css.poster}>
      <h1>ความเห็น {id}</h1>
      <p style={{ fontSize: '0.7rem' }}>{data.timestamp}</p>
      {data.base64Image && <img src={data.base64Image} style={{ width: '100%' }} alt='' />}
      <p>{data.text}</p>
    </div>
  )
}